...
import com.zxl.config.smsConfig.SmsCodeTokenGranter;
...
class AuthorizationServerConfig extends AuthorizationServerConfigurerAdapter {
  ...
  //  private WebResponseExceptionTranslator customOAuth2ResponseExceptionTranslator;
    ...
    private TokenGranter tokenGranter(final AuthorizationServerEndpointsConfigurer endpoints) {
       ...
       // smsCodeTokenGranter.setRedisUtil(redisUtil);
        //这里将userFeign注入到smsCodeTokenGranter类里用，因为如果直接在smsCodeTokenGranter类里@Autowire是无法注入userFeign，因为smsCodeTokenGranter类头不带(比如@Component)的注解，
        smsCodeTokenGranter.setUserFeign(userFeign);
